...
bool Game::Initialize() {
    ...
    mWindow = SDL_CreateWindow("Game Programming in C++ (Exercise 4.2)", 1024, 768, 0);
    if(!mWindow) {
    ...
    SDL_SetPointerProperty(props, SDL_PROP_RENDERER_CREATE_WINDOW_POINTER, mWindow);
    SDL_SetNumberProperty(props, SDL_PROP_RENDERER_CREATE_PRESENT_VSYNC_NUMBER, 1);

    mRenderer = SDL_CreateRendererWithProperties(props);
...
void Game::DrawTexture(SDL_Texture* texture, const Vector2& pos, const Vector2& size) {
    ...
    SDL_RenderTexture(mRenderer, texture, nullptr, &r);
}
